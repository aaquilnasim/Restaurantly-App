...
public class MenuRepository {
	...
	private org.springframework.jdbc.core.RowMapper<Menu> rowMapper= (ResultSet rs, int rowNum)->{
		...
		public void addMenu(Menu m) {
			String sql="INSERT INTO menu(menuID,menuName,menuDescription,menuImage)"
					+ " VALUES ('"+m.getMenuID()+"','"+m.getMenuName()+"','"+m.getMenuDescription()+"','"+m.getMenuImage()+"')";
			jdbctemplate.execute(sql);
		}
		
		public void updateMenu(Menu m,int id) {
			String sql="update menu set menuName='"+m.getMenuName()+"',menuDescription='"+m.getMenuDescription()+"',"
					+ "menuImage='"+m.getMenuImage()+"' where menuID="+m.getMenuID()+"";
			jdbctemplate.execute(sql);
		}
